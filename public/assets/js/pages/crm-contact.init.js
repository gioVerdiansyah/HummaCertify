    ...
    removeBtns && Array.from(removeBtns).forEach(function(e) {
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function() { contactList.remove("id", t.outerHTML), document.getElementById("deleteRecord-close").click() })
            })
        })
...
Array.prototype.slice.call(forms).forEach(function(n) {
    n.addEventListener("submit", function(e) {
        var t, a;
        n.checkValidity() ? (e.preventDefault(), "" === customerNameField.value || "" === company_nameField.value || "" === email_idField.value || "" === phoneField.value || "" === lead_scoreField.value || "" === designationField.value || editlist ? "" !== customerNameField.value && "" !== company_nameField.value && "" !== email_idField.value && "" !== phoneField.value && "" !== lead_scoreField.value && "" !== designationField.value && editlist && (t = contactList.get({ id: idField.value }), Array.from(t).forEach(function(e) {
            var t = (isid = (new DOMParser).parseFromString(e._values.id, "text/html")).body.firstElementChild.innerHTML,
                a = tagInputField.getValue(!0),
                n = "";
            Array.from(a).forEach((e, t) => { n += '<span class="badge bg-primary-subtle text-primary me-1">' + e + "</span>" }), t == itemId && e.values({ id: `<a href="javascript:void(0);" class="fw-medium link-primary">#VZ${idField.value}</a>`, name: '<div class="d-flex align-items-center">                            <div class="flex-shrink-0"><img src="' + customerImg.src + '" alt="" class="avatar-xs rounded-circle object-fit-cover"></div>                            <div class="flex-grow-1 ms-2 name">' + customerNameField.value + "</div>                            </div>", company_name: company_nameField.value, designation: designationField.value, email_id: email_idField.value, phone: phoneField.value, lead_score: lead_scoreField.value, tags: n, date: dateValue + ' <small class="text-muted">' + currentTime() + "</small>" })
