...
var checkAll = document.getElementById("checkAll"),
    ...
    editlist = !1,
    options = { valueNames: ["id", "name", "company_name", "date", "leads_score", "phone", "location", "tags", { attr: "src", name: "image_src" }], page: perPage, pagination: !0, plugins: [ListPagination({ left: 2, right: 2 })] },
...
var isValue = (isCount = (new DOMParser).parseFromString(leadsList.items.slice(-1)[0]._values.id, "text/html")).body.firstElementChild.innerHTML,
    ...
    table = (refreshCallbacks(), document.getElementById("showModal").addEventListener("show.bs.modal", function(e) { e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit Lead", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Update") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Add Lead", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Add Lead") : (document.getElementById("exampleModalLabel").innerHTML = "List Lead", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none") }), ischeckboxcheck(), document.getElementById("showModal").addEventListener("hidden.bs.modal", function() { clearFields() }), document.querySelector("#leadsList").addEventListener("click", function() { ischeckboxcheck() }), document.getElementById("customerTable")),
    ...
    removeBtns && Array.from(removeBtns).forEach(function(e) {
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function() { leadsList.remove("id", t.outerHTML), document.getElementById("deleteRecord-close").click() })
            })
        })
        ...
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                t == itemId && (editlist = !0, idField.value = t, leadImg.src = e._values.image_src, leadNameField.value = e._values.name, company_nameField.value = e._values.company_name, dateField.value = e._values.date, leads_scoreField.value = e._values.leads_score, phoneField.value = e._values.phone, a && Array.from(a).forEach(e => { tagInputField.setChoiceByValue(e.innerHTML) }), locationField.value = e._values.location, flatpickr("#date-field", { dateFormat: "d M, Y", defaultDate: e._values.date }))
            })
        })
    })
}

function clearFields() { leadImg.src = "assets/images/users/user-dummy-img.jpg", leadNameField.value = "", company_nameField.value = "", dateField.value = "", leads_scoreField.value = "", phoneField.value = "", locationField.value = "", tagInputField.removeActiveItems(), tagInputField.setChoiceByValue("0") }
