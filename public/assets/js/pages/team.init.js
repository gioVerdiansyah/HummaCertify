...
function bookmarkBtn() { Array.from(document.querySelectorAll(".favourite-btn")).forEach(function(e) { e.addEventListener("click", function() { e.classList.contains("active") ? e.classList.remove("active") : e.classList.add("active") }) }) }
fetch(url + "team-member-list.json").then(e => e.json()).then(e => { loadTeamData(allmemberlist = e) }).catch(e => console.error(e)), bookmarkBtn();
var editlist = !1;

function editMemberList() {
    var r;
    Array.from(document.querySelectorAll(".edit-list")).forEach(function(t) { t.addEventListener("click", function(e) { r = t.getAttribute("data-edit-id"), allmemberlist = allmemberlist.map(function(e) { return e.id == r && (editlist = !0, document.getElementById("createMemberLabel").innerHTML = "Edit Member", document.getElementById("addNewMember").innerHTML = "Save", "" == e.memberImg ? document.getElementById("member-img").src = "assets/images/users/user-dummy-img.jpg" : document.getElementById("member-img").src = e.memberImg, document.getElementById("cover-img").src = e.coverImg, document.getElementById("memberid-input").value = e.id, document.getElementById("teammembersName").value = e.memberName, document.getElementById("designation").value = e.position, document.getElementById("project-input").value = e.projects, document.getElementById("task-input").value = e.tasks, document.getElementById("memberlist-form").classList.remove("was-validated")), e }) }) })
}

function fetchIdFromObj(e) { return parseInt(e.id) }

function findNextId() { var e, t; return 0 === allmemberlist.length ? 0 : (e = fetchIdFromObj(allmemberlist[allmemberlist.length - 1])) <= (t = fetchIdFromObj(allmemberlist[0])) ? t + 1 : e + 1 }

function sortElementsById() { loadTeamData(allmemberlist.sort(function(e, t) { e = fetchIdFromObj(e), t = fetchIdFromObj(t); return t < e ? -1 : e < t ? 1 : 0 })) }
    ...
    Array.from(document.querySelectorAll(".remove-list")).forEach(function(t) {
        t.addEventListener("click", function(e) {
            r = t.getAttribute("data-remove-id"), document.getElementById("remove-item").addEventListener("click", function() {
                var t;
                t = r, loadTeamData(allmemberlist = allmemberlist.filter(function(e) { return e.id != t })), document.getElementById("close-removeMemberModal").click()
            })
        })
    })
}
...
document.querySelector("#member-image-input").addEventListener("change", function() {
        ...
        var e = document.querySelector("#cover-img"),
            ...
            r = new FileReader;
        r.addEventListener("load", function() { e.src = r.result }, !1), t && r.readAsDataURL(t)
    }), Array.from(document.querySelectorAll(".addMembers-modal")).forEach(function(e) { e.addEventListener("click", function(e) { document.getElementById("createMemberLabel").innerHTML = "Add New Members", document.getElementById("addNewMember").innerHTML = "Add Member", document.getElementById("teammembersName").value = "", document.getElementById("designation").value = "", document.getElementById("cover-img").src = "assets/images/small/img-9.jpg", document.getElementById("member-img").src = "assets/images/users/user-dummy-img.jpg", document.getElementById("memberlist-form").classList.remove("was-validated") }) }),
    function() {
        "use strict";
...
searchMemberList.addEventListener("keyup", function() {
    ...
    t = e;
    var t, e = allmemberlist.filter(function(e) { return -1 !== e.memberName.toLowerCase().indexOf(t.toLowerCase()) || -1 !== e.position.toLowerCase().indexOf(t.toLowerCase()) });
