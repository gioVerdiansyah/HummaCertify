...
var str_dt = function(e) {
        var e = new Date(e),
            ...
            i = "" + e.getDate(),
            e = e.getFullYear();
        return t.length < 2 && (t = "0" + t), [(i = i.length < 2 ? "0" + i : i) + " " + t, e].join(", ")
    },
...
function filterOrder(e) {
    var t = e;
    ticketsList.filter(function(e) { e = (matchData = (new DOMParser).parseFromString(e.values().status, "text/html")).body.firstElementChild.innerHTML; return "All" == e || "All" == t || e == t }), ticketsList.update()
}
...
refreshCallbacks(), document.getElementById("showModal").addEventListener("show.bs.modal", function(e) { e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit Ticket", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Update") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("modal-id").style.display = "none", document.getElementById("exampleModalLabel").innerHTML = "Add Ticket", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Add Ticket") : (document.getElementById("exampleModalLabel").innerHTML = "List Ticket", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none") }), ischeckboxcheck(), document.getElementById("showModal").addEventListener("hidden.bs.modal", function() { clearFields() }), document.querySelector("#ticketsList").addEventListener("click", function() { ischeckboxcheck() });
...
function SearchData() {
    ...
    ticketsList.filter(function(e) {
        var t = (matchData = (new DOMParser).parseFromString(e.values().status, "text/html")).body.firstElementChild.innerHTML,
            ...
            a = !1,
            i = "all" == t || "all" == s || t == s,
            a = new Date(e.values().create_date.slice(0, 12)) >= new Date(n) && new Date(e.values().create_date.slice(0, 12)) <= new Date(d);
        return i && a || (i && "" == l ? i : a && "" == l ? a : void 0)
    }), ticketsList.update()
}
var count = 14,
    forms = document.querySelectorAll(".tablelist-form"),
    example = (Array.prototype.slice.call(forms).forEach(function(i) {
        i.addEventListener("submit", function(e) {
            var t;
            i.checkValidity() ? (e.preventDefault(), "" === tasksTitleField.value || "" === client_nameNameField.value || "" === assignedtoNameField.value || "" === dateField.value || "" === dateDueField.value || "" === statusField.value || "" === priorityField.value || editlist ? "" !== tasksTitleField.value && "" !== client_nameNameField.value && "" !== assignedtoNameField.value && "" !== dateField.value && "" !== dateDueField.value && "" !== statusField.value && "" !== priorityField.value && editlist && (t = ticketsList.get({ id: idField.value }), Array.from(t).forEach(function(e) {
                (isid = (new DOMParser).parseFromString(e._values.id, "text/html")).body.firstElementChild.innerHTML == itemId && e.values({ id: '<a href="javascript:void(0);" onclick="ViewTickets(this)" data-id="' + idField.value + '" class="fw-medium link-primary">' + idField.value + "</a>", tasks_name: tasksTitleField.value, client_name: client_nameNameField.value, create_date: str_dt(dateField.value), due_date: str_dt(dateDueField.value), priority: isPriority(priorityField.value), status: isStatus(statusField.value) })
            }), document.getElementById("close-modal").click(), clearFields(), Swal.fire({ position: "center", icon: "success", title: "Ticket updated Successfully!", showConfirmButton: !1, timer: 2e3, showCloseButton: !0 })) : (ticketsList.add({ id: '<a href="javascript:void(0);" onclick="ViewTickets(this)" data-id="' + count + '" class="fw-medium link-primary ticket-id">#VLZ' + count + "</a>", tasks_name: tasksTitleField.value, client_name: client_nameNameField.value, assignedto: assignedtoNameField.value, create_date: dateField.value, due_date: dateDueField.value, priority: isPriority(priorityField.value), status: isStatus(statusField.value) }), ticketsList.sort("id", { order: "desc" }), document.getElementById("close-modal").click(), clearFields(), refreshCallbacks(), filterOrder("All"), count++, Swal.fire({ position: "center", icon: "success", title: "Ticket inserted successfully!", showConfirmButton: !1, timer: 2e3, showCloseButton: !0 }))) : (e.preventDefault(), e.stopPropagation())
        }, !1)
    ...
    removeBtns && Array.from(removeBtns).forEach(function(e) {
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function() { ticketsList.remove("id", t.outerHTML), document.getElementById("deleteRecord-close").click() })
            })
        })
