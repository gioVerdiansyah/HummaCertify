var checkAll = document.getElementById("checkAll"),
    ...
    perPage = 8,
    editlist = !1,
    options = { valueNames: ["id", "company", "designation", "date", "contacts", "type", "status"], page: perPage, pagination: !0, plugins: [ListPagination({ left: 2, right: 2 })] },
...
function filterOrder(e) {
    var t = e;
    applicationList.filter(function(e) { e = (matchData = (new DOMParser).parseFromString(e.values().status, "text/html")).body.firstElementChild.innerHTML; return "All" == e || "All" == t || e == t }), applicationList.update()
}
Array.from(tabEl).forEach(function(e) { e.addEventListener("shown.bs.tab", function(e) { filterOrder(e.target.id) }) });
var example = new Choices(typeField, { searchEnabled: !1 }),
    statusVal = new Choices(statusField, { searchEnabled: !1 }),
    table = (document.getElementById("showModal").addEventListener("show.bs.modal", function(e) { e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit Application", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Update") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Add Application", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Add Application") : (document.getElementById("exampleModalLabel").innerHTML = "List Application", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none") }), ischeckboxcheck(), document.getElementById("showModal").addEventListener("hidden.bs.modal", function() { clearFields() }), document.querySelector("#applicationList").addEventListener("click", function() { refreshCallbacks(), ischeckboxcheck() }), document.getElementById("jobListTable")),
    ...
    Array.from(removeBtns).forEach(function(e) {
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function() { applicationList.remove("id", t.outerHTML), document.getElementById("deleteRecord-close").click() })
            })
        })
...
Array.prototype.slice.call(forms).forEach(function(a) {
    a.addEventListener("submit", function(e) {
        var t;
        a.checkValidity() ? (e.preventDefault(), "" === companyField.value || "" === designationField.value || "" === dateField.value || "" === contactField.value || "" === statusField.value || "" === typeField.value || editlist ? "" !== companyField.value && "" !== designationField.value && "" !== dateField.value && "" !== contactField.value && "" !== statusField.value && "" !== typeField.value && editlist && (t = applicationList.get({ id: idField.value }), Array.from(t).forEach(function(e) {
            (isid = (new DOMParser).parseFromString(e._values.id, "text/html")).body.firstElementChild.innerHTML == itemId && e.values({ id: '<a href="javascript:void(0);" class="fw-medium link-primary">' + idField.value + "</a>", company: '<div class="d-flex align-items-center">                            <div class="flex-shrink-0">                                <img src="' + companyLogoImg.src + '" alt="" class="avatar-xxs rounded-circle image_src object-fit-cover">                            </div>                            <div class="flex-grow-1 ms-2 name">' + companyField.value + "</div>                        </div>", designation: designationField.value, date: dateField.value, contacts: contactField.value, type: typeField.value, status: isStatus(statusField.value) }), document.getElementById("close-modal").click()
        }), Swal.fire({ position: "center", icon: "success", title: "Application updated Successfully!", showConfirmButton: !1, timer: 2e3, showCloseButton: !0 })) : (applicationList.add({ id: '<a href="#" class="fw-medium link-primary">#VZ' + count + "</a>", company: '<div class="d-flex align-items-center">                    <div class="flex-shrink-0">                        <img src="' + companyLogoImg.src + '" alt="" class="avatar-xxs rounded-circle image_src object-fit-cover">                    </div>                    <div class="flex-grow-1 ms-2 name">' + companyField.value + "</div>                </div>", designation: designationField.value, date: dateField.value, contacts: contactField.value, type: typeField.value, status: isStatus(statusField.value) }), applicationList.sort("id", { order: "desc" }), document.getElementById("close-modal").click(), clearFields(), refreshCallbacks(), filterOrder("All"), count++, Swal.fire({ position: "center", icon: "success", title: "Application inserted successfully!", showConfirmButton: !1, timer: 2e3, showCloseButton: !0 }))) : (e.preventDefault(), e.stopPropagation())
    }, !1)
