...
projectModalEl.addEventListener("show.bs.modal", e => { document.getElementById("projectname-input").value = "", document.querySelectorAll(".createProject-form").forEach(function(e) { e.classList.remove("was-validated") }) });
var editList = !1,
    statusVal = (flatpickr("#task-duedate-input", { dateFormat: "d M, Y" }), Array.from(document.getElementsByClassName("createTask")).forEach(function(e) { e.addEventListener("click", function() { document.getElementById("createTaskLabel").innerHTML = "Create Task", document.getElementById("addNewTodo").innerHTML = "Add Task", clearFields() }) }), statusField = document.getElementById("task-status-input"), new Choices(statusField, { searchEnabled: !1 })),
    priorityVal = (priorityField = document.getElementById("priority-field"), new Choices(priorityField, { searchEnabled: !1 }));

function fetchIdFromObj(e) { return parseInt(e.id) }

function findNextId() { var e, t; return 0 === todoList.length ? 0 : (e = fetchIdFromObj(todoList[todoList.length - 1])) <= (t = fetchIdFromObj(todoList[0])) ? t + 1 : e + 1 }

function clearFields() { document.getElementById("task-title-input").value = "", document.getElementById("task-duedate-input").value = "", document.getElementById("total-assignee").innerHTML = "0", Array.from(document.querySelectorAll(".select-element .dropdown-menu .dropdown-item.active")).forEach(function(e) { e.classList.remove("active") }), Array.from(document.querySelectorAll("#assignee-member .avatar-group-item")).forEach(function(e) { e.remove() }), document.getElementById("task-error-msg").style.display = "none", statusVal.removeActiveItems(), statusVal.setChoiceByValue("New"), priorityVal.removeActiveItems(), priorityVal.setChoiceByValue("") }

function sortElementsById() { load(todoList.sort(function(e, t) { e = fetchIdFromObj(e), t = fetchIdFromObj(t); return t < e ? -1 : e < t ? 1 : 0 })) }

function sortElementsByName() { load(todoList.sort(function(e, t) { e = e.todo.toLowerCase(), t = t.todo.toLowerCase(); return e < t ? -1 : t < e ? 1 : 0 })) }
Array.from(document.getElementsByClassName("select-element")).forEach(function(e) {
    ...
    }), "" === s || "" === i || "" === n || editList ? "" !== s && "" !== i && "" !== n && editList && (t = 0, t = document.getElementById("taskid-input").value, todoList = todoList.map(function(e) { return e.id == t ? ("Completed" == statusVal.getValue(!0) ? e.checkedElem = !0 : e.checkedElem = !1, { id: document.getElementById("taskid-input").value, checkedElem: e.checkedElem, todo: s, assignedto: o, dueDate: a, status: i, priority: n }) : e }), editList = !1, document.getElementById("createTaskBtn-close").click()) : (r = findNextId(), todoList.push({ id: r, checkedElem: !1, todo: s, assignedto: o, dueDate: a, status: i, priority: n }), sortElementsById(), document.getElementById("createTaskBtn-close").click()), load(todoList), !0)
}), document.getElementById("createTask").addEventListener("hidden.bs.modal", e => { clearFields() });
var searchTaskList = document.getElementById("searchTaskList");

function loadList(e) { document.getElementById("elmLoader").innerHTML = "", drawList(e) }
searchTaskList.addEventListener("keyup", function() {
    var e = searchTaskList.value.toLowerCase();
    t = e;
    var t, e = todoList.filter(function(e) { return -1 !== e.todo.toLowerCase().indexOf(t.toLowerCase()) });
...
var drake = dragula([document.getElementById("task-list")], { moves: function(e, t, s) { return s.classList.contains("task-handle") } }),
    scroll = autoScroll([document.querySelector("#todo-content")], { margin: 20, maxSpeed: 100, scrollWhenOutside: !0, autoScroll: function() { return this.down && drake.dragging } });
...
function checkTaskStatus() {
    Array.from(document.querySelectorAll("#task-list tr")).forEach(function(e) {
        e.querySelector(".form-check .form-check-input").addEventListener("change", function() {
            var t = this.value;
            load(todoList = this.checked ? todoList.map(function(e) { return e.id == t && (e.checkedElem = !0, e.status = "Completed"), e }) : todoList.map(function(e) { return e.id == t && (e.checkedElem = !1, e.status = "Inprogress"), e }))
        })
    })
}
    ...
    Array.from(document.querySelectorAll(".edit-list")).forEach(function(t) {
        t.addEventListener("click", function(e) {
            s = t.getAttribute("data-edit-id"), todoList = todoList.map(function(e) {
                var t;
                return e.id == s && (editList = !0, document.getElementById("createTaskLabel").innerHTML = "Edit Task", document.getElementById("addNewTodo").innerHTML = "Save", document.getElementById("taskid-input").value = e.id, document.getElementById("task-title-input").value = e.todo, flatpickr("#task-duedate-input", { dateFormat: "d M, Y", defaultDate: e.dueDate }), t = (new DOMParser).parseFromString(e.status, "text/html").body, statusVal.setChoiceByValue(t.innerHTML), t = (new DOMParser).parseFromString(e.priority, "text/html").body, priorityVal.setChoiceByValue(t.innerHTML), Array.from(document.querySelectorAll(".select-element .dropdown-menu ul li a")).forEach(function(s) {
                    var a = s.querySelector(".flex-grow-1").innerHTML;
                    e.assignedto.map(function(e) { var t; return e.assigneeName == a && (s.classList.add("active"), t = document.getElementById("assignee-member"), s.classList.contains("active")) && (folderlisthtml = '<a href="javascript: void(0);" class="avatar-group-item mb-2" data-img="' + e.assigneeImg + '"  data-bs-toggle="tooltip" data -bs-placement="top" data-bs-title="' + e.assigneeName + '">                                    <img src="' + e.assigneeImg + '" alt="" class="rounded-circle avatar-xs" />                                    </a>', t.insertAdjacentHTML("beforeend", folderlisthtml), tooltipElm()), s })
...
var taskStatusInput = new Choices(document.getElementById("choices-select-status"), { searchEnabled: !1 }),
    ...
    }, !1), new Choices(document.getElementById("choices-select-sortlist"), { searchEnabled: !1 }));

function load(e) { loadList(e) }
taskSortListInput.passedElement.element.addEventListener("change", function(e) { e = e.detail.value; "By ID" == e ? sortElementsById() : "By Name" == e ? sortElementsByName() : load(todoList) }, !1), window.onload = function() { sortElementsById() };