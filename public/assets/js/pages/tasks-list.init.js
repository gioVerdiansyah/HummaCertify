var checkAll = document.getElementById("checkAll"),
    ...
    editlist = !1,
    options = { valueNames: ["id", "project_name", "tasks_name", "client_name", "assignedto", "due_date", "status", "priority"], page: perPage, pagination: !0, plugins: [ListPagination({ left: 2, right: 2 })] },
...
function filterOrder(e) {
    var t = e;
    tasksList.filter(function(e) { e = (matchData = (new DOMParser).parseFromString(e.values().status, "text/html")).body.firstElementChild.innerHTML; return "All" == e || "All" == t || e == t }), tasksList.update()
}
...
refreshCallbacks(), document.getElementById("showModal").addEventListener("show.bs.modal", function(e) { e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit Task", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Update") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Add Task", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Add Task") : (document.getElementById("exampleModalLabel").innerHTML = "List Task", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none") }), document.getElementById("showModal").addEventListener("hidden.bs.modal", function() { clearFields() }), document.querySelector("#tasksList").addEventListener("click", function() { ischeckboxcheck() });
var table = document.getElementById("tasksTable"),
    ...
    example = (Array.prototype.slice.call(forms).forEach(function(a) {
        a.addEventListener("submit", function(e) {
            var t;
            a.checkValidity() ? (e.preventDefault(), "" === projectNameField.value || "" === tasksTitleField.value || "" === clientNameField.value || "" === dateDueField.value || "" === priorityField.value || "" === statusField.value || editlist ? "" !== projectNameField.value && "" !== tasksTitleField.value && "" !== clientNameField.value && "" !== dateDueField.value && "" !== priorityField.value && "" !== statusField.value && editlist && (t = tasksList.get({ id: idField.value }), Array.from(t).forEach(function(e) {
                (isid = (new DOMParser).parseFromString(e._values.id, "text/html")).body.firstElementChild.innerHTML == itemId && e.values({ id: '<a href="javascript:void(0);" class="fw-medium link-primary">' + idField.value + "</a>", project_name: '<a href="apps-projects-overview.html" class="fw-medium link-primary">' + projectNameField.value + "</a>", tasks_name: tasksTitleField.value, client_name: clientNameField.value, assignedto: assignToUsers(), due_date: fomateDate(dateDueField.value), status: isStatus(statusField.value), priority: isPriority(priorityField.value) })
            }), document.getElementById("close-modal").click(), clearFields(), tooltipElm(), Swal.fire({ position: "center", icon: "success", title: "Task updated Successfully!", showConfirmButton: !1, timer: 2e3, showCloseButton: !0 })) : (tasksList.add({ id: '<a href="apps-tasks-details.html" class="fw-medium link-primary">#VLZ' + count + "</a>", project_name: '<a href="apps-projects-overview.html" class="fw-medium link-primary">' + projectNameField.value + "</a>", tasks_name: tasksTitleField.value, client_name: clientNameField.value, assignedto: assignToUsers(), due_date: fomateDate(dateDueField.value), status: isStatus(statusField.value), priority: isPriority(priorityField.value) }), tasksList.sort("id", { order: "desc" }), document.getElementById("close-modal").click(), clearFields(), refreshCallbacks(), tooltipElm(), count++, Swal.fire({ position: "center", icon: "success", title: "Task inserted successfully!", showConfirmButton: !1, timer: 2e3, showCloseButton: !0 }))) : (e.preventDefault(), e.stopPropagation())
        }, !1)
    ...
    removeBtns && Array.from(removeBtns).forEach(function(e) {
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function() { tasksList.remove("id", t.outerHTML), document.getElementById("deleteRecord-close").click() })
            })
        })
        ...
        e.addEventListener("click", function(e) {
            ...
            Array.from(e).forEach(function(e) {
                ...
                a == itemId && (editlist = !0, idField.value = a, a = (project = (new DOMParser).parseFromString(e._values.project_name, "text/html")).body.firstElementChild.innerHTML, statusVal.setChoiceByValue(t), projectNameField.value = a, tasksTitleField.value = e._values.tasks_name, clientNameField.value = e._values.client_name, dateDueField.value = e._values.due_date, Array.from(document.querySelectorAll('input[name="assignedTo[]"]')).forEach(function(t) {
                    ...
                    (new DOMParser).parseFromString(e._values.assignedto, "text/html").querySelectorAll(".avatar-group .avatar-group-item").forEach(function(e) { e.getAttribute("data-bs-title") == a && (t.checked = !0) })
                }), statusVal && statusVal.destroy(), statusVal = new Choices(statusField, { searchEnabled: !1 }), t = (val = (new DOMParser).parseFromString(e._values.status, "text/html")).body.firstElementChild.innerHTML, statusVal.setChoiceByValue(t), example && example.destroy(), example = new Choices(priorityField, { searchEnabled: !1 }), a = (val = (new DOMParser).parseFromString(e._values.priority, "text/html")).body.firstElementChild.innerHTML, example.setChoiceByValue(a), flatpickr("#duedate-field", { dateFormat: "d M, Y", defaultDate: e._values.due_date }))
            })
        })
    })
}

function clearFields() { projectNameField.value = "", tasksTitleField.value = "", clientNameField.value = "", assignedtoNameField.value = "", dateDueField.value = "", document.querySelectorAll('input[name="assignedTo[]"]').forEach(function(e) { e.checked = !1 }), example && example.destroy(), example = new Choices(priorityField), statusVal && statusVal.destroy(), statusVal = new Choices(statusField) }
...
function assignToUsers() {
    ...
    return Array.from(e.slice(0, 3)).forEach(function(e) { t += '<a href="javascript: void(0);" class="avatar-group-item" data-img="' + e.assigneeImg + '"  data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="' + e.assigneeName + '">            <img src="' + e.assigneeImg + '" alt="" class="rounded-circle avatar-xxs" />        </a>' }), 3 < e.length && (e = e.length - 3, t += '<a href="javascript: void(0);" class="avatar-group-item"  data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="' + e + ' More">        <div class="avatar-xxs">        <div class="avatar-title rounded-circle">' + e + "+</div>        </div>    </a>"), t += "</div>"
}
...
function deleteMultiple() {
    ...
    }), document.getElementById("checkAll").checked = !1) : Swal.fire({ title: "Please select at least one checkbox", confirmButtonClass: "btn btn-info", buttonsStyling: !1, showCloseButton: !0 })
}
